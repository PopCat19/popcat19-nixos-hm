  ...
  programs.fish.shellInit = ''
    ...
    function nixos-commit-rebuild-push
        ...
        if test "$deprecated_flag_used" = "true"
            echo "⚠️  Remote builds are deprecated and have been removed. Ignoring remote flags and proceeding with local build."
        end

        if test -z "$commit_message"
            echo "❌ Usage: nixos-commit-rebuild-push '<commit-message>'"
            echo "💡 Example: nixos-commit-rebuild-push 'update config'"
            cd $original_dir
            return 1
        end
