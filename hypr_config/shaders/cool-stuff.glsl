...
// [Eris's Additions Parameters]
#define LENS_DISTORTION_STRENGTH -0.15 // Lens distortion: >0 pincushion, <0 barrel
#define FILM_GRAIN_INTENSITY 0.04 // Intensity of the film grain
#define NOISE_BAR_HEIGHT 0.08 // Height of the rolling noise bar
#define NOISE_BAR_SPEED 0.4 // Vertical speed of the noise bar
#define LIGHT_LEAK_INTENSITY 0.35 // Intensity of the light leaks
...
vec2 pixelate(vec2 uv) {
  ...
  vec2 grid = vec2(
    PIXEL_GRID_SIZE,
    PIXEL_GRID_SIZE * (VIGNETTE_ASPECT.y / VIGNETTE_ASPECT.x)
  );
...
float computeVignette(vec2 uv) {
  ...
  return smoothstep(
    VIGNETTE_RADIUS,
    VIGNETTE_RADIUS - VIGNETTE_SMOOTHNESS,
    dist
  );
}
...
vec3 calculateBloom(vec2 uv) {
  ...
  for (int i = 0; i < BLOOM_SAMPLES; i++) {
    ...
    float weight = exp(-(radius * radius) * BLOOM_FALLOFF_CURVE) *
      softThreshold;
...
vec3 applyVHSOverlay(vec2 uv, float time, vec3 originalColor) {
  vec2 displacement = VHS_OVERLAY_OFFSET *
    VHS_OVERLAY_DISPLACEMENT_SCALE *
  ...
  return mix(
    originalColor,
    overlay *
      VHS_OVERLAY_TINT_RGB *
      (1.0 + sin(time * VHS_OVERLAY_FREQ) * VHS_OVERLAY_WAVE_AMPLITUDE),
...
vec3 applyGlitch(vec2 uv, float time, vec3 color, float isActive) {
  ...
  float blockGlitch = step(
    random(vec2(blockY, currentInterval)),
    GLITCH_BLOCK_PROBABILITY
  );
  float displacement = (random(vec2(blockY, time)) - 0.5) *
    GLITCH_DISPLACEMENT;
  ...
  finalColor = mix(
    finalColor,
    texture2D(tex, displacedUV).rgb,
    isActive * blockGlitch
  );

  return finalColor;
}

// --- [NEW] Eris's Aesthetic Functions ---
...
vec2 applyLensDistortion(vec2 uv) {
  ...
  float distortion = 1.0 + r2 * LENS_DISTORTION_STRENGTH;
  return 0.5 + centeredUV * distortion;
#else
  return uv;
#endif
}
...
vec3 applyFilmGrain(vec3 color, vec2 uv, float time) {
  ...
  return color + color * noise * FILM_GRAIN_INTENSITY;
#else
  return color;
#endif
}

// Analog Rolling Noise Bar
vec3 applyNoiseBar(vec3 color, vec2 uv, float time) {
#if DEBUG_NOISE_BAR
  float barY = fract(time * NOISE_BAR_SPEED);
  float mask = smoothstep(
    NOISE_BAR_HEIGHT,
    0.0,
    abs(uv.y - barY)
  ) - smoothstep(NOISE_BAR_HEIGHT * 1.2, 0.0, abs(uv.y - barY - 0.01));
  if (mask > 0.0) {
    vec2 displacedUV = uv;
    displacedUV.x += (random(vec2(uv.y, time)) - 0.5) * 0.03 * mask;
    color = mix(color, texture2D(tex, displacedUV).rgb, mask * 0.5);
    color += (random(vec2(uv.x * 10.0, time)) - 0.5) * 0.2 * mask;
  }
  return color;
#else
  return color;
...
// Light Leaks
vec3 applyLightLeaks(vec3 color, vec2 uv, float time) {
#if DEBUG_LIGHT_LEAKS
  float leak1_pos = sin(time * 0.1) * 1.5;
  vec3 leak1_col = vec3(1.0, 0.2, 0.1);
  float leak1 =
    smoothstep(0.0, 0.5, length(uv - vec2(leak1_pos, 0.2))) * 0.5;
  color += leak1 * leak1_col * LIGHT_LEAK_INTENSITY;

  float leak2_pos = cos(time * 0.25) * 1.2;
  vec3 leak2_col = vec3(0.1, 0.3, 1.0);
  float leak2 =
    smoothstep(0.0, 0.6, length(uv - vec2(leak2_pos, 0.8))) * 0.4;
  color += leak2 * leak2_col * LIGHT_LEAK_INTENSITY;
...
void main() {
  ...
  // Apply lens distortion first to warp the coordinate space
  vec2 distortedUV = applyLensDistortion(v_texcoord);
  vec2 processedUV = distortedUV;
  ...
  float xVibActive = step(seedXVib, GLITCH_X_VIBRATION_PROBABILITY) *
    intervalActive;
  ...
  // --- Apply Eris's Final Touches ---

  // Apply rolling noise bar
#if DEBUG_NOISE_BAR
  color = applyNoiseBar(color, distortedUV, time);
#endif

  // Apply light leaks
#if DEBUG_LIGHT_LEAKS
  color = applyLightLeaks(color, distortedUV, time);
#endif

  // Apply film grain as the final layer
#if DEBUG_FILM_GRAIN
  color = applyFilmGrain(color, distortedUV, time);
#endif
