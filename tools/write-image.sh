...
validate_device() {
  ...
  [[ "$t" == disk ]] || { err "$dev is TYPE=$t, not a whole disk."; return 1; }
  pk="$(pk_of "$dev")"
  is_system_pk "$pk" && { err "Refusing to write to a system disk ($pk)."; return 1; }
}
...
main() {
  ...
  while (($#)); do
    case "$1" in
      installer|klipper) ARG_TYPE="$1"; shift;;
      -d) ARG_DEV="${2:-}"; shift 2;;
      -i) ARG_IMAGE="${2:-}"; shift 2;;
      -y) ARG_YES=true; shift;;
      -n) ARG_DRY=true; shift;;
      -N) ARG_NOBUILD=true; shift;;
      -h) usage; exit 0;;
  ...
  [[ -f "$write_path" ]] || { err "Image not found: $write_path"; exit 3; }
  ...
  ((img_bytes > dev_bytes)) && { err "Image ($img_human) larger than target ($dev_human)."; exit 7; }

  $ARG_DRY && { info "[DRY-RUN] dd if=$write_path of=$ARG_DEV bs=4M status=progress conv=fdatasync"; exit 0; }
